...
meParser::meParser(char *conf)
{
  ...
  // Load grammar
  G = std::make_unique<Grammar>(path, sym_rec);
}
...
void meParser::loadSymRec(char *config)
{
  ...
  duration.emplace(dur_path, max_strokes, sym_rec);
...
void meParser::initCYKterms(Sample &M, TableCYK *tcyk, int N, int K)
{
  ...
  for (int i = 0; i < M.nStrokes(); i++)
  {
    ...
    bool insertar = false;
    for (std::list<ProductionT *>::iterator it = G->prodTerms.begin(); it != G->prodTerms.end(); it++)
    {
      ProductionT *prod = *it;
      ...
      for (int k = 0; k < NB; k++)
        if (pr[k] > 0.0 && prod->getClase(clase[k]) && prod->getPrior(clase[k]) > -FLT_MAX)
        {
          ...
          float prob = log(InsPen) + ptfactor * prod->getPrior(clase[k]) + qfactor * log(pr[k]) + dfactor * log(duration->prob(clase[k], 1));
...
void meParser::combineStrokes(Sample &M, TableCYK *tcyk, LogSpace **LSP, int N)
{
  ...
  for (int stkc1 = 1; stkc1 < N; stkc1++)
  {
    ...
    for (int size = 2; size <= min(max_strokes, N); size++)
    {
      ...
      for (std::list<int>::iterator it = close_list.begin(); it != close_list.end(); it++)
        stkvec[VS++] = *it;
      ...
      for (int i = size - 2; i < VS; i++)
      {
        ...
        //  for std::list<int>::iterator it=stks_list.begin(); it!=stks_list.end(); it++)
        //    printf(" %d", *it);
        //  printf(" }\n");
  ...
  for (int talla = 2; talla <= N; talla++)
  {
    ...
    for (int a = 1; a < talla; a++)
    {
      ...
      for (CellCYK *c1 = tcyk.get(a); c1; c1 = c1->sig)
      {
        ...
        for (std::list<CellCYK *>::iterator c2 = c1setH.begin(); c2 != c1setH.end(); c2++)
        {

          for (std::list<ProductionB *>::iterator it = G->prodsH.begin(); it != G->prodsH.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
          ...
          for (std::list<ProductionB *>::iterator it = G->prodsSup.begin(); it != G->prodsSup.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
          ...
          for (std::list<ProductionB *>::iterator it = G->prodsSub.begin(); it != G->prodsSub.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
        ...
        for (std::list<CellCYK *>::iterator c2 = c1setV.begin(); c2 != c1setV.end(); c2++)
        {

          for (std::list<ProductionB *>::iterator it = G->prodsV.begin(); it != G->prodsV.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
          ...
          // prodsVe
          for (std::list<ProductionB *>::iterator it = G->prodsVe.begin(); it != G->prodsVe.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
        ...
        for (std::list<CellCYK *>::iterator c2 = c1setU.begin(); c2 != c1setU.end(); c2++)
        {
        ...
        for (std::list<CellCYK *>::iterator c2 = c1setI.begin(); c2 != c1setI.end(); c2++)
        {

          for (std::list<ProductionB *>::iterator it = G->prodsIns.begin(); it != G->prodsIns.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
        ...
        // Mroot
        for (std::list<CellCYK *>::iterator c2 = c1setM.begin(); c2 != c1setM.end(); c2++)
        {

          for (std::list<ProductionB *>::iterator it = G->prodsMrt.begin(); it != G->prodsMrt.end(); it++)
          {
            if ((*it)->prior == -FLT_MAX)
              continue;
        ...
        for (int pps = 0; pps < c1->nnt; pps++)
        {
          ...
          if (c1->noterm[pps] && c1->noterm[pps]->prod && c1->noterm[pps]->prod->tipo() == 'B')
          {
            ...
            for (std::list<CellCYK *>::iterator c2 = c1setS.begin(); c2 != c1setS.end(); c2++)
            {

              if ((*c2)->x == c1->x && c1 != *c2)
              {

                for (std::list<ProductionB *>::iterator it = G->prodsSSE.begin(); it != G->prodsSSE.end(); it++)
                {
                  if ((*it)->prior == -FLT_MAX)
                    continue;
                  ...
                  if (c1->noterm[pa] && (*c2)->noterm[pb] && c1->noterm[pa]->prod && (*c2)->noterm[pb]->prod && c1->noterm[pa]->hi == (*c2)->noterm[pb]->hi && c1->noterm[pa]->prod->tipo() == 'B' && (*c2)->noterm[pb]->prod->tipo() == 'P' && c1->noterm[pa]->hd->parent->compatible((*c2)->noterm[pb]->hd->parent))
                  {
                    ...
                    if (max((*it)->solape(c1->noterm[pa]->hd, (*c2)->noterm[pb]->hd),
                            (*it)->solape((*c2)->noterm[pb]->hd, c1->noterm[pa]->hd)) > 0.1)
                      continue;
