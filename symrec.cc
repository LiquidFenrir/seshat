...
int SymRec::classify(Sample &M, SegmentHyp &SegHyp, const int NB, int *vclase, float *vpr, int *as, int *ds)
{
  ...
  for (const auto it_idx : SegHyp.stks)
  {
    ...
    for (int j = 0, je = cur_stroke.getNpuntos(); j < je; ++j)
    {
  ...
  *ds = (regy + SegHyp.cen) / 2;
  ...
  std::vector<pair<float, int>> clason(NB), clasoff(NB), clashyb(2 * NB);
  ...
  BLSTMclassification(blstm_on, *feat_on, clason);
  BLSTMclassification(blstm_off, *feat_off, clasoff);
  ...
  std::sort(&clashyb[0], &clashyb[hybnext], std::greater<pair<float, int>>());
...
void SymRec::BLSTMclassification(Mdrnn *net, const DataSequence &seq, std::span<pair<float, int>> claspr)
{
