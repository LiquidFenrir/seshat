...
struct Rprop : public DataExporter, public Optimiser {
    ...
    Rprop(
        ...
        std::vector<real_t>& derivatives, WeightContainer* weight, DataExportHandler* deh, bool on = false)
    ...
    void update_weights()
    {
        ...
        if (online) {
            ...
            if (avgDelta > prevAvgDelta) {
                etaPlus = std::max((real_t)1.0, etaPlus - etaChange);
            } else {
                etaPlus += etaChange;
            }
