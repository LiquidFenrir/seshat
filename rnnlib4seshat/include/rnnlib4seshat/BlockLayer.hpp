...
struct BlockLayer : public Layer {
    ...
    std::vector<int> inCoords;
    size_t sourceSize;
    CVI blockIterator;
    std::vector<size_t> outSeqShape;

    // functions
    BlockLayer(Layer* src, const std::vector<size_t>& blockshape, WeightContainer* weight, DataExportHandler* deh)
        : Layer(
    ...
    void start_sequence()
    {
        for (int i = 0; i < outSeqShape.size(); ++i) {
            outSeqShape.at(i) = ceil(
                (real_t)this->source->output_seq_shape().at(i) / (real_t)blockShape.at(i));
        }
        outputActivations.reshape(outSeqShape, 0);
        outputErrors.reshape(outputActivations, 0);
    }
    void feed_forward(const std::vector<int>& outCoords)
    {
    ...
    void feed_back(const std::vector<int>& outCoords)
    {
        ...
        for (blockIterator.begin(); !blockIterator.end; ++blockIterator) {
            ...
            if (inErr) {
                transform(outIt, outIt + sourceSize, inErr, inErr, std::plus<real_t>());
            }
            outIt += sourceSize;
        }
    }
