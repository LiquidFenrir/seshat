...
struct LstmLayer : public Layer {
    ...
    std::vector<std::vector<int>> stateDelays;
    std::vector<int> delayedCoords;
    std::vector<View<real_t>> oldStates;
    std::vector<View<real_t>> nextErrors;
    std::vector<View<real_t>> nextFgActs;
    std::vector<View<real_t>> nextCellErrors;
#ifdef PEEPS
    LstmLayer<CI, CO, G>* peepSource;
    std::pair<size_t, size_t> peepRange;
#endif

    // functions
    LstmLayer(
        const std::string& name, const std::vector<int>& directions, size_t nb, WeightContainer* wc, DataExportHandler* deh,
    ...
    void feed_forward(const std::vector<int>& coords)
    {
        ...
        LOOP(int b, iota_range(numBlocks))
        {
            ...
            std::transform(
                ...
                actBegin + cellStart, std::bind(std::multiplies<real_t>(), std::placeholders::_1, outGateAct));
            cellStart = cellEnd;
    ...
    void feed_back(const std::vector<int>& coords)
    {
        ...
        LOOP(int b, iota_range(numBlocks))
        {
            ...
            // output gate error
            real_t outGateError = G::deriv(outGateAct) * std::inner_product(preOutGateActBegin + cellStart, preOutGateActBegin + cellEnd, outputErrorBegin + cellStart, 0.0);
            ...
            // input gate error
            *errorIt = G::deriv(inGateAct) * std::inner_product(cellErrorBegin + cellStart, cellErrorBegin + cellEnd, preGateStateBegin + cellStart, 0.0);
            ++errorIt;
            ...
            LOOP(int d, iota_range(this->num_seq_dims()))
            {
                const View<real_t>& os = oldStates[d];
                if (os.size() != 0) {
                    *errorIt = G::deriv(forgetGateActBegin[fgStart + d]) * std::inner_product(cellErrorBegin + cellStart, cellErrorBegin + cellEnd, os.begin() + cellStart, 0.0);
                } else {
                    *errorIt = 0;
                }
                ++errorIt;
...
#ifdef PEEPS
    void update_derivs(const std::vector<int>& coords)
    {
