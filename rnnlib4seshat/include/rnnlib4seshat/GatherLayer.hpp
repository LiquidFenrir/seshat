...
struct GatherLayer : public Layer {
    // data
    std::vector<Layer*> sources;

    // functions
    GatherLayer(const std::string& name, std::vector<Layer*>& srcs, WeightContainer* weight, DataExportHandler* deh)
    ...
    int get_size(std::vector<Layer*>& srcs)
    {
        int size = 0;
