...
struct InputLayer : public Layer {
    // functions
    InputLayer(const std::string& name, size_t numSeqDims, size_t size, const std::vector<std::string>& inputLabels, WeightContainer* weight, DataExportHandler* deh)
        : Layer(name, numSeqDims, 0, size, weight, deh)
    {
        // const std::vector<std::string>* labs = inputLabels.empty() ? 0 : &inputLabels;
