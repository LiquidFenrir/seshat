...
struct IdentityLayer : public FlatLayer {
    // functions
    IdentityLayer(const std::string& name, size_t numSeqDims, size_t size, WeightContainer* weight, DataExportHandler* deh)
        : FlatLayer(name, numSeqDims, size, weight, deh)
    {
        display(this->outputErrors, "errors");
        display(this->outputActivations, "activations");
    }
    IdentityLayer(const std::string& name, const std::vector<int>& directions, size_t size, WeightContainer* weight, DataExportHandler* deh)
