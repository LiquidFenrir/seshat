...
struct SteepestDescent : public DataExporter, public Optimiser {
    ...
    // functions
    SteepestDescent(
        const std::string& name, std::ostream& o, std::vector<real_t>& weights,
        std::vector<real_t>& derivatives, WeightContainer* weight, DataExportHandler* deh, real_t lr = 1e-4, real_t mom = 0.9)
    ...
    void update_weights()
    {
        ...
        if (verbose) {
            out << this->name << " weight updates:" << std::endl;
