...
struct Trainer : public DataExporter {
    ...
    std::map<std::string, real_t>& netErrors;
    const std::map<std::string, real_t>& netNormFactors;
    std::vector<real_t> distortedWeights;
    std::string task;
    real_t dataFraction;
    ...
    std::map<std::string, real_t> mdlSeqErrors;

    // functions
    Trainer(
        std::ostream& o, Mdrnn* n, ConfigFile& conf, WeightContainer* wc, DataExportHandler* deh, const std::string& name = "trainer")
        ...
        , task(config.get<std::string>("task"))
        ...
        , batchLearn(config.get<bool>(
              "batchLearn", (config.get<std::string>("optimiser", "steepest") == "rprop") && (seqsPerWeightUpdate == 1)))
        ,
        // trainFiles(
        //     config.get_list<std::string>("trainFile"), task, !batchLearn,
        //     dataFraction),
        // testFiles(config.get_list<std::string>("testFile"), task, false, dataFraction),
        // valFiles(config.get_list<std::string>("valFile"), task, false, dataFraction),
        ...
        , mdlOptimiser(0)
    {
        std::string optType = config.get<std::string>("optimiser", "steepest");
        std::string optName = "weight_optimiser";
        ...
        if (mdl) {
            std::string mdlOptType = config.get<std::string>("mdlOptimiser", optType);
            std::string mdlOptName = "mdl_dev_optimiser";
            if (mdlOptType == "rprop") {
    ...
    bool print_distortions()
    {
        ...
        if (inputNoiseDev != 0) {
            out << "adding noise to input data, std dev " << inputNoiseDev << std::endl
                << std::endl;
            distortions = true;
        }
        if (weightDistortion) {
            out << "adding  noise to weights every sequence, std dev "
                << weightDistortion << std::endl
                << std::endl;
            distortions = true;
        }
        if (mdl) {
            prt_line(out);
            out << "MDL training" << std::endl;
            PRINT(mdlWeight, out);
    ...
    //   if (trainFiles.size()) {
    //     out << "training data:" << std::endl << trainFiles;
    //   }
    //   if (valFiles.size()) {
    //     prt_line(out);
    //     out << "validation data:" << std::endl << valFiles;
    //   }
    //   if (testFiles.size())       {
    //     prt_line(out);
    //     out << "test data:" << std::endl << testFiles;
    //   }
    //   out << std::endl;
    // }

    void save_data(const std::string& filename, ConfigFile& conf)
    {
        std::ofstream fout(filename.c_str());
        if (fout.is_open()) {
            out << "saving to " << filename << std::endl;
            config.set_val<bool>("loadWeights", true);
            // fout << config << DataExportHandler::instance();
        } else {
            out << "WARNING trainer unable to save to file " << filename << std::endl;
        }
    }
    ...
    /*bool check_for_best(
        ...
        std::map<std::string, std::pair<int, DatasetErrors> >& bestErrors, int epoch) {
      bool newBest = false;
      LOOP(const PSD& p, currentErrors.errors) {
        const std::string& errName = p.first;
        real_t err = p.second;
    ...
    /*void print_best_errors(
        ...
        const std::map<std::string, std::pair<int, DatasetErrors> >& bestErrors) const {
      out << name << " set errors for best networks" << std::endl;
      LOOP(const PSPIDE& p, bestErrors) {
        prt_line(out);
        out << "epoch " << p.second.first << " (" << p.first << ")" << std::endl
            << p.second.second;
      }
    }*/
    ...
    //     if (verbose) {
    //       out << "data sequence:" << std::endl;
    //       out << "file = " << data.dataset->filename << std::endl;
    //       out << "index = " << data.seqIndex << std::endl;
    //       out << *seq;
    //     }
    //     net->calculate_errors(*seq);
