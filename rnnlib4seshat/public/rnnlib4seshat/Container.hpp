...
template<class T>
struct View : public std::span<T> {
    View(const std::pair<T*, T*>& p)
        : std::span<T>(p.first, p.second) { }
    View(T* first, T* second)
        : std::span<T>(first, second) { }
    View()
        : std::span<T>() { }
    View slice(int first = 0, int last = std::numeric_limits<int>::max())
    {
    ...
    const View slice(int first = 0, int last = std::numeric_limits<int>::max()) const
    {
        return slice(first, last);
    }
    const View slice(std::pair<int, int>& r) const
    {
        return slice(r.first, r.second);
    }
    T& at(size_t i)
    ...
    template<class R>
    const View<T>& operator=(const R& r) const
    {
        check(std::size(r) == this->size(), "can't assign range " + str(r) + " to view " + str(*this));
        copy(r, *this);
        return *this;
    }
    /*
...
struct Vector : public std::vector<T> {
    ...
    {
        this->resize(std::max((size_t)0, this->size() - length));
        return *this;
    }
    ...
    View<T> slice(int first = 0, int last = std::numeric_limits<int>::max())
    {
        first = bound(first, 0, (int)this->size());
        if (last < 0) {
            last += (int)this->size();
    ...
    View<T> slice(std::pair<int, int>& r)
    {
        return slice(r.first, r.second);
    }
    const View<T> slice(
        int first = 0, int last = std::numeric_limits<int>::max()) const
    {
        return slice(first, last);
    }
    const View<T> slice(std::pair<int, int>& r) const
    {
        return slice(r.first, r.second);
    }
    template<class R>
    Vector<T>& extend(const R& r)
    {
    ...
    template<class R>
    Vector<T>& operator=(const R& r)
    {
        std::vector<T>::resize(std::size(r));
        copy(r, *this);
        return *this;
    }
    template<class T2>
...
struct Set : public std::set<T> {
    ...
    template<class R>
    Set<T>& operator=(const R& r)
    {
        this->clear();
        return this->extend(r);
    }
    template<class R>
    Set<T>& extend(const R& r)
    {
