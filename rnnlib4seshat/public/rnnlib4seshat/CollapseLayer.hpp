...
struct CollapseLayer : public Layer {
    // data
    std::vector<bool> activeDims;
    std::vector<size_t> outSeqShape;

    // functions
    CollapseLayer(Layer* src, Layer* des, WeightContainer* weight, DataExportHandler* deh, const std::vector<bool>& activDims = {})
    ...
    std::vector<int> get_out_coords(const std::vector<int>& inCoords)
    {
