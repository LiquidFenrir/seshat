...
struct NeuronLayer : public FlatLayer {
    NeuronLayer(const std::string& name, size_t numDims, size_t size, WeightContainer* weight, DataExportHandler* deh)
        : FlatLayer(name, numDims, size, weight, deh)
    {
        init();
    }
    NeuronLayer(const std::string& name, const std::vector<int>& directions, size_t size, WeightContainer* weight, DataExportHandler* deh)
