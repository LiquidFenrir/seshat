...
struct Layer : public DataExporter {
    ...
    // functions
    Layer(
        const std::string& name, size_t numSeqDims, size_t inputSize,
    ...
    Layer(
        const std::string& name, const std::vector<int>& dirs, size_t inputSize,
        size_t outputSize, WeightContainer* weight, DataExportHandler* deh, Layer* src = 0)
        : DataExporter(name, deh)
        , directions(dirs)
        , inputActivations(inputSize)
    ...
    virtual const View<real_t> out_acts(const std::vector<int>& coords)
    {
        return outputActivations[coords];
    }

    virtual const View<real_t> out_errs(const std::vector<int>& coords)
    {
...
struct FlatLayer : public Layer {
    FlatLayer(const std::string& name, size_t numSeqDims, size_t size, WeightContainer* weight, DataExportHandler* deh, Layer* src = 0)
        : Layer(name, numSeqDims, size, size, weight, deh, src) { }

    FlatLayer(
        const std::string& name, const std::vector<int>& dirs, size_t size, WeightContainer* weight, DataExportHandler* deh, Layer* src = 0)
