...
void Sample::setRegion(CellCYK *c, std::list<int> *LT) {
  ...
  for(float l=0.0; l < 1.0; l += dl) {
    ...
    for(int i=y-1; i<=y+1; i++)
      for(int j=x-1; j<=x+1; j++) {
	      ...
	      if( stkid>=0 ) pix_stk.img[i * pix_stk.width + j] = stkid;
      }
  }
}
...
void Sample::renderStrokesPBM(const std::vector<int> &SL, VectorImage& img) {
...
bool Sample::visibility(std::list<int> *strokes_list) {

  map<int,bool> visited;
  for (std::list<int>::iterator it=strokes_list->begin(); it!=strokes_list->end(); it++)
    visited[*it] = false;
...
void Sample::get_close_strokes(int id, std::list<int> *L, float dist_th) {
  ...
  for (std::list<int>::iterator it=auxlist.begin(); it!=auxlist.end(); it++)
    L->push_back( *it );
}
